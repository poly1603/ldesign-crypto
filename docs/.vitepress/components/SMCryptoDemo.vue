  ...
  <div class="sm-crypto-demo">
    ...
    <div class="algorithm-section">
      ...
      <div class="form-group">
        <button @click="generateSM2Keys" :disabled="isLoading" class="btn-primary">
      ...
      <div v-if="sm2Keys" class="key-display">
        <div class="key-item">
          ...
          <textarea :value="sm2Keys.publicKey" readonly></textarea>
          <button @click="copyToClipboard(sm2Keys.publicKey)" class="btn-copy">📋</button>
        </div>
        <div class="key-item">
          <label>🔐 SM2私钥 (用于解密和签名):</label>
          <textarea :value="sm2Keys.privateKey" readonly></textarea>
          <button @click="copyToClipboard(sm2Keys.privateKey)" class="btn-copy">📋</button>
        </div>
      </div>

      <div class="operation-tabs">
        <button
          :class="['tab-btn', { active: sm2Operation === 'encrypt' }]"
          @click="sm2Operation = 'encrypt'"
        >
          🔒 加密/解密
        </button>
        <button
          :class="['tab-btn', { active: sm2Operation === 'sign' }]"
      ...
      <div v-if="sm2Operation === 'encrypt'" class="operation-panel">
        ...
        <div class="button-group">
          <button @click="encryptSM2" :disabled="isLoading || !sm2Keys" class="btn-primary">
            🔒 SM2加密
          </button>
          <button @click="decryptSM2" :disabled="isLoading || !sm2Keys || !sm2Result" class="btn-secondary">
            🔓 SM2解密
          </button>
        </div>
      </div>
      ...
      <div v-if="sm2Operation === 'sign'" class="operation-panel">
        <div class="form-group">
          <label>待签名数据:</label>
          <textarea v-model="sm2SignData" placeholder="输入要签名的数据"></textarea>
        </div>
        ...
        <div class="button-group">
          <button @click="signSM2" :disabled="isLoading || !sm2Keys" class="btn-primary">
            ✍️ SM2签名
          </button>
          <button @click="verifySM2" :disabled="isLoading || !sm2Keys || !sm2Signature" class="btn-secondary">
            ✅ 验证签名
          </button>
        </div>
        ...
        <div v-if="sm2Signature" class="signature-display">
          <label>数字签名:</label>
          <textarea :value="sm2Signature" readonly></textarea>
          <button @click="copyToClipboard(sm2Signature)" class="btn-copy">📋</button>
        </div>
      </div>

      <div v-if="sm2Result" class="result-panel">
        <h4>SM2 操作结果:</h4>
        <div class="result-content">{{ sm2Result }}</div>
        <button @click="copyToClipboard(sm2Result)" class="btn-copy">📋 复制</button>
      </div>
    </div>
    ...
    <div class="algorithm-section">
      ...
      <div class="form-group">
        <label>待哈希数据:</label>
        <textarea v-model="sm3Data" placeholder="输入要计算SM3哈希的数据"></textarea>
      </div>
      ...
      <div class="button-group">
        <button @click="calculateSM3" :disabled="isLoading" class="btn-primary">
          🔍 计算SM3哈希
        </button>
      </div>
      ...
      <div v-if="sm3Result" class="result-panel">
        <h4>SM3 哈希值 (256位):</h4>
        <div class="result-content hash-result">{{ sm3Result }}</div>
        <button @click="copyToClipboard(sm3Result)" class="btn-copy">📋 复制</button>
        <div class="hash-info">
    ...
    <div class="algorithm-section">
      ...
      <div class="form-group">
        ...
        <div class="key-input">
          ...
          <button @click="generateSM4Key" class="btn-generate">生成</button>
        </div>
      </div>
      ...
      <div class="form-group">
        <label>待加密数据:</label>
        <textarea v-model="sm4Data" placeholder="输入要加密的数据"></textarea>
      </div>

      <div class="button-group">
        <button @click="encryptSM4" :disabled="isLoading" class="btn-primary">
          🔒 SM4加密
        </button>
        <button @click="decryptSM4" :disabled="isLoading || !sm4Result" class="btn-secondary">
          🔓 SM4解密
        </button>
      </div>
      ...
      <div v-if="sm4Result" class="result-panel">
        <h4>SM4 操作结果:</h4>
        <div class="result-content">{{ sm4Result }}</div>
        <button @click="copyToClipboard(sm4Result)" class="btn-copy">📋 复制</button>
      </div>
    </div>
    ...
    <div class="algorithm-section">
      ...
      <div class="form-group">
        <label>重要数据:</label>
        <textarea v-model="comprehensiveData" placeholder="输入需要安全传输的重要数据"></textarea>
      </div>

      <div class="button-group">
        <button @click="comprehensiveEncrypt" :disabled="isLoading" class="btn-primary">
          🛡️ 安全加密 (SM2+SM4+SM3)
        </button>
        <button @click="comprehensiveDecrypt" :disabled="isLoading || !comprehensiveResult" class="btn-secondary">
          🔓 安全解密
        </button>
      </div>
      ...
      <div v-if="comprehensiveResult" class="comprehensive-result">
        ...
        <div class="result-item">
          ...
          <div class="result-content">{{ comprehensiveResult.encryptedData?.substring(0, 100) }}...</div>
        </div>
        <div class="result-item">
          <label>加密密钥 (SM2):</label>
          <div class="result-content">{{ comprehensiveResult.encryptedKey?.substring(0, 100) }}...</div>
        </div>
        <div class="result-item">
          <label>完整性校验 (SM3):</label>
          <div class="result-content">{{ comprehensiveResult.integrity }}</div>
        </div>
      </div>
    </div>
...
const encryptSM2 = async () => {
  if (!crypto || !sm2Keys.value) return
...
const decryptSM2 = async () => {
  if (!crypto || !sm2Keys.value || !sm2Result.value) return
...
const verifySM2 = async () => {
  if (!crypto || !sm2Keys.value || !sm2Signature.value) return
...
const decryptSM4 = async () => {
  if (!crypto || !sm4Result.value) return
...
// 综合应用
const comprehensiveEncrypt = async () => {
  if (!crypto) return
...
const comprehensiveDecrypt = async () => {
  if (!crypto || !comprehensiveResult.value) return
