...
class KeyRotation {
  ...
  encrypt(data, _algorithm = "AES", options) {
    ...
    const result = aes.aes.encrypt(data, activeKey.key, options);
    return {
      ...result,
  ...
  decrypt(encryptedData, _algorithm = "AES", options) {
    ...
    return aes.aes.decrypt(encryptedData, keyInfo.key, options);
  }
  /**
   * 重新加密数据（使用新的激活密钥）
   */
...
exports.createKeyRotation = createKeyRotation;
//# sourceMappingURL=key-rotation.cjs.map
