...
require('crypto-js');
require('./errors.cjs');
require('./key-derivation.cjs');
require('./object-pool.cjs');
require('./performance-logger.cjs');
...
class Benchmark {
  ...
  async benchmarkAES() {
    ...
    for (const size of this.options.dataSizes) {
      ...
      const encrypted = aes.aes.encrypt(data, key);
      const decryptStart = performance.now();
      for (let i = 0; i < this.options.iterations; i++) {
        aes.aes.decrypt(encrypted, key);
      }
  ...
  async benchmarkEncoding(type) {
    ...
    for (const size of this.options.dataSizes) {
      const data = this.generateTestData(size);
      for (let i = 0; i < this.options.warmupIterations; i++) {
        encoding.encoding.encode(data, type);
      }
      ...
      const encoded = encoding.encoding.encode(data, type);
      for (let i = 0; i < this.options.warmupIterations; i++) {
        encoding.encoding.decode(encoded, type);
      }
...
exports.compareBenchmark = compareBenchmark;
