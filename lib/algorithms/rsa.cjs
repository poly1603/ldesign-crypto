...
class RSAEncryptor {
  constructor() {
    this.defaultOptions = {
      keyFormat: "pkcs1",
      keySize: index.CONSTANTS.RSA.DEFAULT_KEY_SIZE,
      padding: "OAEP"
    };
   ...
   * 生成 RSA 密钥对
   */
  generateKeyPair(keySize = index.CONSTANTS.RSA.DEFAULT_KEY_SIZE) {
    try {
      if (!index.CONSTANTS.RSA.KEY_SIZES.includes(keySize)) {
        throw index.ErrorUtils.createEncryptionError(`Unsupported RSA key size: ${keySize}`, "RSA");
      }
      ...
      throw index.ErrorUtils.createEncryptionError("Failed to generate RSA key pair", "RSA");
    }
  }
  /**
   * RSA 公钥加密
  ...
  encrypt(data, publicKey, options = {}) {
    try {
      if (index.ValidationUtils.isEmpty(data)) {
        throw index.ErrorUtils.createEncryptionError("Data cannot be empty", "RSA");
      }
      if (index.ValidationUtils.isEmpty(publicKey)) {
        throw index.ErrorUtils.createEncryptionError("Public key cannot be empty", "RSA");
      }
      ...
      throw index.ErrorUtils.createEncryptionError("Unknown encryption error", "RSA");
    }
  }
  /**
   * RSA 私钥解密
  ...
  decrypt(encryptedData, privateKey, options = {}) {
    const opts = { ...this.defaultOptions, ...options };
    try {
      if (index.ValidationUtils.isEmpty(privateKey)) {
        throw index.ErrorUtils.createDecryptionError("Private key cannot be empty", "RSA");
      }
      let ciphertext;
  ...
  sign(data, privateKey, algorithm = "sha256") {
    try {
      ...
      if (index.ValidationUtils.isEmpty(privateKey)) {
        throw index.ErrorUtils.createEncryptionError("Private key cannot be empty", "RSA");
      }
      const privateKeyObj = this.parsePrivateKey(privateKey);
      ...
      throw index.ErrorUtils.createEncryptionError("Failed to sign data", "RSA");
    }
  }
  /**
   * RSA 验证签名
  ...
  verify(data, signature, publicKey, algorithm = "sha256") {
    try {
      if (index.ValidationUtils.isEmpty(data) || index.ValidationUtils.isEmpty(signature) || index.ValidationUtils.isEmpty(publicKey)) {
        return false;
      }
      const publicKeyObj = this.parsePublicKey(publicKey);
      const md = this.getMessageDigest(algorithm);
...
-----END PUBLIC KEY-----`;
      ...
      throw index.ErrorUtils.createEncryptionError("Invalid public key format", "RSA");
    }
  }
  /**
   * 解析私钥
...
-----END PRIVATE KEY-----`;
      ...
      throw index.ErrorUtils.createDecryptionError("Invalid private key format", "RSA");
    }
  }
  /**
   * 获取填充方案
...
exports.RSAEncryptor = RSAEncryptor;
exports.rsa = rsa;
