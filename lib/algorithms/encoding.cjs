...
class Encoder {
  ...
  encode(data, encoding2) {
    try {
      switch (encoding2.toLowerCase()) {
        ...
        // UTF-8 是默认编码，无需转换
        default:
          throw index.ErrorUtils.createEncryptionError(`Unsupported encoding type: ${encoding2}`, "Encoding");
      }
    } catch (error) {
      if (error instanceof Error) {
        throw error;
      }
      throw index.ErrorUtils.createEncryptionError("Unknown encoding error", "Encoding");
    }
  }
  /**
   * 解码字符串
  ...
  decode(encodedData, encoding2) {
    try {
      switch (encoding2.toLowerCase()) {
        ...
        // UTF-8 是默认编码，无需转换
        default:
          throw index.ErrorUtils.createDecryptionError(`Unsupported encoding type: ${encoding2}`, "Decoding");
      }
    } catch (error) {
      if (error instanceof Error) {
        throw error;
      }
      throw index.ErrorUtils.createDecryptionError("Unknown decoding error", "Decoding");
    }
  }
  /**
   * Base64 编码
  ...
  encodeBase64(data) {
    try {
      ...
      throw index.ErrorUtils.createEncryptionError("Failed to encode Base64", "Base64");
    }
  }
  /**
   * Base64 解码
  ...
  decodeBase64(encodedData) {
    try {
      if (!index.ValidationUtils.isValidBase64(encodedData)) {
        throw index.ErrorUtils.createDecryptionError("Invalid Base64 format", "Base64");
      }
      ...
      throw index.ErrorUtils.createDecryptionError("Failed to decode Base64", "Base64");
    }
  }
  /**
   * Hex 编码
  ...
  encodeHex(data) {
    try {
      ...
      throw index.ErrorUtils.createEncryptionError("Failed to encode Hex", "Hex");
    }
  }
  /**
   * Hex 解码
  ...
  decodeHex(encodedData) {
    try {
      if (!index.ValidationUtils.isValidHex(encodedData)) {
        throw index.ErrorUtils.createDecryptionError("Invalid Hex format", "Hex");
      }
      return CryptoJS.enc.Utf8.stringify(CryptoJS.enc.Hex.parse(encodedData));
    } catch {
      throw index.ErrorUtils.createDecryptionError("Failed to decode Hex", "Hex");
    }
  }
  /**
   * URL 安全的 Base64 编码
