...
import { CONSTANTS, ValidationUtils, ErrorUtils, RandomUtils } from '../utils/index.js';
...
class AESEncryptor {
  ...
  tryDecryptWithDefaultFormat(encryptedData, key, mode) {
    try {
      try {
        ...
        throw ErrorUtils.createDecryptionError("Invalid encrypted data format - not valid Base64", "AES");
      }
  ...
  validateDecryptionResult(decrypted, decryptedString) {
    ...
    if (!this.validateUtf8String(decryptedString)) {
      throw ErrorUtils.createDecryptionError("Decrypted data contains invalid characters - wrong key", "AES");
    }
  }
  /**
   * 创建解密错误结果
...
AESEncryptor.keyCache = new LRUCache({
  maxSize: 100,
  ttl: 5 * 60 * 1e3,
  updateAgeOnGet: true,
