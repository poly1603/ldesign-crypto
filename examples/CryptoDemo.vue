...
import { useCrypto, useHash, useSM, useSymmetricCrypto } from '../src/vue'
  ...
  <div class="crypto-demo">
    ...
    <div class="demo-section">
      ...
      <div class="button-group">
        <button :disabled="symmetricLoading" class="btn-primary" @click="encryptSymmetric">
          {{ symmetricLoading ? '加密中...' : '加密' }}
        </button>
        <button :disabled="symmetricLoading" class="btn-secondary" @click="decryptSymmetric">
          {{ symmetricLoading ? '解密中...' : '解密' }}
        </button>
      </div>
      <div v-if="symmetricError" class="error">
        {{ symmetricError }}
      </div>
    ...
    <div class="demo-section">
      ...
      <button :disabled="hashLoading" class="btn-primary" @click="calculateHash">
        {{ hashLoading ? '计算中...' : '计算哈希' }}
      </button>
      <div v-if="hashError" class="error">
        {{ hashError }}
      </div>
    ...
    <div class="demo-section">
      <h2>🇨🇳 国密SM2算法</h2>
      <div class="form-group">
        <button :disabled="sm2Loading" class="btn-generate" @click="generateSM2Keys">
      ...
      <div v-if="sm2KeyPair" class="key-pair">
        <div class="key-item">
          <label>公钥:</label>
          <textarea :value="sm2KeyPair.publicKey" readonly />
        </div>
        <div class="key-item">
          <label>私钥:</label>
          <textarea :value="sm2KeyPair.privateKey" readonly />
        </div>
      </div>
      <div class="form-group">
        <label>待加密数据:</label>
      ...
      <div class="button-group">
        <button :disabled="sm2Loading || !sm2KeyPair" class="btn-primary" @click="encryptSM2">
          {{ sm2Loading ? '加密中...' : 'SM2加密' }}
        </button>
        <button :disabled="sm2Loading || !sm2KeyPair" class="btn-secondary" @click="decryptSM2">
          {{ sm2Loading ? '解密中...' : 'SM2解密' }}
        </button>
      </div>
      <div v-if="sm2Error" class="error">
        {{ sm2Error }}
      </div>
    ...
    <div class="demo-section">
      ...
      <div class="tool-grid">
        <div class="tool-item">
          ...
          <button class="btn-primary" @click="generateRandomString">
            生成
          </button>
        ...
        <div class="tool-item">
          ...
          <button class="btn-primary" @click="generateKey">
            生成密钥
          </button>
          <div v-if="keyResult" class="result">
            {{ keyResult }}
          </div>
        </div>
      </div>
    ...
    <div class="demo-section">
      <h2>📊 性能统计</h2>
      <button class="btn-secondary" @click="refreshMetrics">
        刷新统计
      </button>
